   ...
   if { [info exists data_series($w,$series,x)] } {
       ...
       if { $type == "line" || $type == "both" } {
          $w create line $pxold $pyold $pxcrd $pycrd \
                         -fill $colour -width 1 -tag [list data data_$series]

	#############Rectangle criation and binding process
	######## $VolArea::VolArea.c.tbr is a table in the aplication window.
        #########note: must have a package require Plotchart 1.8  in the main file.
	 set mi [expr {$pxcrd - 20}]
	 set ma [expr {$pxcrd + 10}]
	 set myi [expr {$pycrd - 20}]
	 set mya [expr {$pycrd + 10}]
	 set object [$w create rectangle $mi $myi $ma $mya -fill $colour -outline $colour -width 6 -tag  [list data data_$series]]

	 $w bind $object <1> [list $VolArea::VolArea.c.ftb.tbr insert end "$series $xcrd $ycrd"]
       }
   }
